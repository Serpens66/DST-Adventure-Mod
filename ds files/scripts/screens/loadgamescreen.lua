...
function LoadGameScreen:MakeSaveTile(slotnum)
	...
	if character and mode and CapyDLC then
		if SaveGameIndex:OwnsMode("survival", slotnum) then
			if SaveGameIndex:OwnsMode("shipwrecked", slotnum) then
				if SaveGameIndex:ROGEnabledOnSWSlot(slotnum) then
					SetShield("SWDLC_icon.tex")
				else
					SetShield("SWBase_icon.tex")
				end
			else
				if SaveGameIndex:ROGEnabledOnSWSlot(slotnum) then
					SetShield("DLCicon.tex")
...
function LoadGameScreen:OnClickTile(slotnum)
	...
	if not SaveGameIndex:GetCurrentMode(slotnum) then
		...
		if self.menu.items[slotnum].dlcindicator then
			TheFrontEnd:PushScreen(SlotDetailsScreen(slotnum, self.menu.items[slotnum].dlcindicator.texture))
		else
