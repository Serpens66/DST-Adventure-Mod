...
local function dostew(inst)
	...
	if stewercmp.product ~= nil then 
		...
		stewercmp.spoiltask = stewercmp.inst:DoTaskInTime(stewercmp.spoiltime, dospoil)
	end
...
function Stewer:OnLoad(data)
    ...
    elseif data.done then
		self.product_spoilage = data.product_spoilage or 1
		self.done = true
		self.targettime = data.timesincefinish and (GetTime() - data.timesincefinish) or (GetTime() - 1)
		self.product = data.product
		...
		if self.spoiltargettime then
			self.spoiltask = self.inst:DoTaskInTime(data.spoiltime, dospoil)
		end
		if self.inst.components.container then		
			self.inst.components.container.canbeopened = false
		end
