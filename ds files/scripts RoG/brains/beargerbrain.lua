    ...
    while not target do
        if not target then
            for k,v in pairs(ents) do
                if v and ((honeypass and v:HasTag("honeyed")) or not honeypass) and v:IsOnValidGround() and inst.components.eater:CanEat(v) and not (v.components.inventoryitem and v.components.inventoryitem:IsHeld()) then
                    target = v
                    break
                end
            end
